...
func main() {
	...
	whitelister, err := bridgetokenwhitelist.NewBridgeTokenWhitelister(
		bridgetokenwhitelist.NewConfig(
			...
			bridgetokenwhitelist.WithL2Host(cliConfig.l2Host),
			bridgetokenwhitelist.WithL2HTTPPort(cliConfig.l2HTTPPort),
			bridgetokenwhitelist.WithL2WSPort(cliConfig.l2WSPort),
