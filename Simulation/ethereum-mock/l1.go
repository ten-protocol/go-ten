...
func NewMiner(id common.NodeId, cfg L1MiningConfig, agg common.NotifyNewBlock, network common.L1Network, statsCollector common.StatsCollector) L1Miner {
	return L1Miner{Id: id, cfg: cfg, Agg: agg, network: network, runCh1: make(chan bool), p2pCh: make(chan *common.Block), miningCh: make(chan *common.Block), canonicalCh: make(chan *common.Block), runCh2: make(chan bool), mempoolCh: make(chan *common.L1Tx), StatsCollector: statsCollector}
}
		...
		case p2pb := <-m.p2pCh: // Received from peers
			if p2pb.Height() > head.Height() {
				if !common.IsAncestor(*head, *p2pb) {
					...
					common.Log(fmt.Sprintf("> M%d: Reorg new=b_%d(%d), old=b_%d(%d), fork=b_%d(%d)", m.Id, p2pb.RootHash().ID(), p2pb.Height, head.RootHash().ID(), head.Height, fork.RootHash().ID(), fork.Height))
				}
				head = m.setHead(p2pb)
			}
...
func (m L1Miner) printBlock(mb *common.Block) string {
...
func (m L1Miner) setHead(b *common.Block) *common.Block {
	m.Agg.RPCNewHead(*b)
	m.canonicalCh <- b
	return b
}
...
func (m L1Miner) P2PReceiveBlock(b *common.Block) {
...
func (m L1Miner) L1P2PGossipTx(tx *common.L1Tx) {
	m.mempoolCh <- tx
}
