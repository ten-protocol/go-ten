...
func LCA(ctx context.Context, newCanonical *types.Header, oldCanonical *types.Header, resolver *blockResolverInMem) (*common.ChainFork, error) {
...
func internalLCA(ctx context.Context, newCanonical *types.Header, oldCanonical *types.Header, resolver *blockResolverInMem, canonicalPath []common.L1BlockHash, nonCanonicalPath []common.L1BlockHash) (*types.Header, []common.L1BlockHash, []common.L1BlockHash, error) {
	...
	if newCanonical.Number.Uint64() > oldCanonical.Number.Uint64() {
		...
		return internalLCA(ctx, p.Header(), oldCanonical, resolver, append(canonicalPath, newCanonical.Hash()), nonCanonicalPath)
	}
	if oldCanonical.Number.Uint64() > newCanonical.Number.Uint64() {
		...
		return internalLCA(ctx, newCanonical, p.Header(), resolver, canonicalPath, append(nonCanonicalPath, oldCanonical.Hash()))
	}
	...
	return internalLCA(ctx, parentBlockA.Header(), parentBlockB.Header(), resolver, append(canonicalPath, newCanonical.Hash()), append(nonCanonicalPath, oldCanonical.Hash()))
}
