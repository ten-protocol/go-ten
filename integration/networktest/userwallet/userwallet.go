...
func (s *UserWallet) SendFunds(ctx context.Context, addr gethcommon.Address, value *big.Int) error {
	err := s.EnsureClientSetup(ctx)
	if err != nil {
		return fmt.Errorf("unable to prepare client to send funds - %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("unable to fetch receipt - %w", err)
	}
	if receipt.Status != types.ReceiptStatusSuccessful {
		return fmt.Errorf("unsuccessful receipt for funds transaction, status=%d", receipt.Status)
	}
...
// if no client is set, this creates an authenticated RPC client (with a viewing key generated, signed and registered).
