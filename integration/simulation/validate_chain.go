...
func checkEthereumBlockchainValidity(t *testing.T, s *Simulation, params *SimParams, efficiencies EfficiencyThresholds) uint64 {
	...
	heights := make([]uint64, len(s.EthNodes))
	for i, node := range s.EthNodes {
...
func checkBlockchainOfEthereumNode(t *testing.T, node exec.EthNode, minHeight uint64, s *Simulation, efficiencies EfficiencyThresholds) uint64 {
	head, height := node.FetchHeadBlock()

	if height < minHeight {
		t.Errorf("Node %d. There were only %d blocks mined. Expected at least: %d.",
			obscurocommon.ShortAddress(node.Info().ID),
			height,
			minHeight,
		)
	}
	...
	if totalDeposited != s.Stats.totalDepositedAmount {
		t.Errorf("Node %d. Deposit amounts don't match. Found %d , expected %d",
			obscurocommon.ShortAddress(node.Info().ID),
			totalDeposited,
			s.Stats.totalDepositedAmount,
		)
	}
	...
	if efficiency > efficiencies.L1EfficiencyThreshold {
		t.Errorf("Node %d. Efficiency in L1 is %f. Expected:%f. Height: %d.",
			obscurocommon.ShortAddress(node.Info().ID),
			efficiency,
			efficiencies.L1EfficiencyThreshold,
			height,
		)
	}
...
func extractDataFromEthereumChain(head *types.Block, node exec.EthNode, s *Simulation) ([]common.Hash, []obscurocommon.L2RootHash, uint64) {
