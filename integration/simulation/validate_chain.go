...
func checkBlockchainOfEthereumNode(t *testing.T, node ethadapter.EthClient, minHeight uint64, s *Simulation) uint64 {
	...
	if totalDepositedLogged.Cmp(totalDeposited) != 0 {
		t.Errorf("Node %d: Logged deposits do not match extracted deposits from bridge. Events logged %d; Deposit count %d", nodeAddr, eventCount, len(deposits))
	}
...
func checkBlockchainOfObscuroNode(t *testing.T, rpcHandles *network.RPCHandles, minObscuroHeight uint64, maxEthereumHeight uint64, s *Simulation, wg *sync.WaitGroup, heights []uint64, nodeIdx int) {
	...
	/*	totalAmountLogged := getLoggedWithdrawals(minObscuroHeight, nodeClient, h)

		if totalAmountLogged.Cmp(totalSuccessfullyWithdrawn) != 0 {
			t.Errorf("Node %d: Logged withdrawals do not match!", nodeAddr)
		} */
...
func getLoggedWithdrawals(minObscuroHeight uint64, nodeClient rpc.Client, currentHeader *common.Header) *big.Int {
	totalAmountLogged := big.NewInt(0)
	for i := minObscuroHeight; i < currentHeader.Number.Uint64(); i++ {
		method := rpc.GetRollupHeaderByNumber
		var result *common.Header
		err := nodeClient.Call(&result, method)
		if err != nil {
			panic(err)
		}

		for _, msg := range result.CrossChainMessages {
			contractAbi, err := abi.JSON(strings.NewReader(erc20.EthERC20MetaData.ABI))
			if err != nil {
				panic(err)
			}
