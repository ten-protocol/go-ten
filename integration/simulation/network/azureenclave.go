...
func (n *networkWithAzureEnclaves) Create(params *params.SimParams, stats *stats.Stats) ([]ethclient.EthClient, []*obscuroclient.Client, []string) {
	...
	params.TxHandler = ethereum_mock.NewMockTxHandler()
	...
	for i := 0; i < len(n.enclaveAddresses); i++ {
		...
		agg := createSocketObscuroNode(int64(i), isGenesis, params.AvgGossipPeriod, stats, nodeP2pAddrs[i], nodeP2pAddrs, n.enclaveAddresses[i], obscuroClientAddr, ethereum_mock.NewMockTxHandler())
	...
	for i := len(n.enclaveAddresses); i < params.NumberOfNodes; i++ {
		...
		_, err := enclave.StartServer(enclaveAddress, nodeID, params.TxHandler, false, false, nil, stats)
		if err != nil {
		...
		obscuroClientAddr := fmt.Sprintf("%s:%d", Localhost, params.StartPort+200+i)
		miner := createMockEthNode(int64(i), params.NumberOfNodes, params.AvgBlockDuration, params.AvgNetworkLatency, stats)
		agg := createSocketObscuroNode(int64(i), false, params.AvgGossipPeriod, stats, nodeP2pAddrs[i], nodeP2pAddrs, enclaveAddress, obscuroClientAddr, ethereum_mock.NewMockTxHandler())
		obscuroClient := obscuroclient.NewClient(obscuroClientAddr)
