...
func (s *InMemDevNetwork) AllocateFaucetFunds(account gethcommon.Address) error {
	// ensure only one test account is getting faucet funds at a time, faucet client isn't thread-safe
	s.faucetLock.Lock()
	defer s.faucetLock.Unlock()

	err := s.faucet.SendFunds(context.Background(), account, _defaultFaucetAmount)
	if err != nil {
		return err
	}
	return nil
...
func (s *InMemDevNetwork) startNodes() {
	if s.obscuroSequencer == nil {
		...
		s.obscuroValidators = make([]*InMemNodeOperator, 0)
