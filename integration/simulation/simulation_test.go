...
func validateL2WithdrawalStats(t *testing.T, node *host.Node, stats *Stats, l2Height uint, txManager *TransactionManager) uint64 {
	...
	// expected condition : some FetchRollupTxs (stats) did not make it to the blockchain
	// best condition : all FetchRollupTxs (stats) were issue and consumed in the blockchain
	// can't happen : sum of headers withdraws greater than issued FetchRollupTxs (stats)
