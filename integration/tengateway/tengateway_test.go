...
func testSessionKeyExpirationAndFundRecovery(t *testing.T, _ int, httpURL, wsURL string, w wallet.Wallet) {
	...
	// Simple print to verify the test is running
	fmt.Println("testSessionKeyExpirationAndFundRecovery: Test is running successfully!")
	testlog.Logger().Info("testSessionKeyExpirationAndFundRecovery: Test is running successfully!")
	...
	// 3) Check initial balance of session key
	...
	// 5) Wait for session key expiration (default is 10 seconds, wait 20 seconds to be safe)
	t.Logf("⏳ Waiting for session key expiration (20 seconds)...")
	time.Sleep(20 * time.Second)
	t.Logf("✓ Session key should now be expired")

	// 6) Check session key balance after expiration (should be 0 due to fund recovery)
	finalBalance, err := user0.HTTPClient.BalanceAt(ctx, skAddress, nil)
	require.NoError(t, err)
	t.Logf("✓ Final session key balance: %s TEN", finalBalance.String())

	// 6.5) If balance is not 0, wait a bit more and check again (transaction might be pending)
	if finalBalance.Cmp(big.NewInt(0)) != 0 {
		t.Logf("⚠️ Session key still has balance, waiting 10 more seconds for fund recovery transaction...")
		time.Sleep(10 * time.Second)
		finalBalance, err = user0.HTTPClient.BalanceAt(ctx, skAddress, nil)
		require.NoError(t, err)
		t.Logf("✓ Final session key balance after additional wait: %s TEN", finalBalance.String())
	}
	...
	// 8) Verify that the session key balance is 0 (funds were recovered)
	require.Equal(t, big.NewInt(0), finalBalance, "Session key balance should be 0 after fund recovery")
