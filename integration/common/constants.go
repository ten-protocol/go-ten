...
// WETH9Bytecode is the compiled bytecode for the canonical WETH9 contract
// This must match the bytecode in go/config/defaults/0-base-config.yaml
...
// WETHAddress is the predefined address where WETH9 is deployed at genesis
var WETHAddress = gethcommon.HexToAddress("0x1000000000000000000000000000000000000042")

// genesisContracts returns the list of contracts to deploy at genesis
func genesisContracts() []tengenesis.Contract {
	return []tengenesis.Contract{
		{
			Address:  WETHAddress,
...
// GenesisContractsOnlyJSON returns genesis JSON with only contracts (WETH9), no prefunded accounts
func GenesisContractsOnlyJSON() string {
	gen := &tengenesis.Genesis{
		Accounts:  []tengenesis.Account{},
		Contracts: genesisContracts(),
	}
...
// TestnetGenesisJSON returns genesis JSON with prefunded accounts and contracts (WETH9)
