...
WORKDIR /home/obscuro/go-obscuro/testnet/launcher/l2contractdeployer/cmd

# Build the binary (output name is optional but useful)
RUN --mount=type=cache,target=/root/.cache/go-build \
    go build -o l2contractdeployer

# --- STAGE 2: Slim runtime image ---
FROM alpine:3.18

# Copy built binary into the runtime image
COPY --from=builder /home/obscuro/go-obscuro/testnet/launcher/l2contractdeployer/cmd/l2contractdeployer /usr/local/bin/l2contractdeployer

# Set working directory (optional)
WORKDIR /usr/local/bin

# Default command to run the script
ENTRYPOINT ["/usr/local/bin/l2contractdeployer"]