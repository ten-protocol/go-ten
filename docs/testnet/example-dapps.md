...
[A number guessing game](https://testnet-obscuroscan.uksouth.azurecontainer.io/game.html) is a simple way of showcasing the principles of privacy in Obscuro. The goal of the game is to guess a secret number. Each time an attempt is made an entrance fee of 1 token is paid. If a player correctly guesses the number the contract will pay out all of the accumulated entrance fees to them and reset itself with a new random number.
...
1. Visit `http://localhost:3000/viewingkeys/` to generate a new viewing key and sign the viewing key when prompted by MetaMask.
1. Browse to [the number guessing game](https://testnet-obscuroscan.uksouth.azurecontainer.io/game.html). Check you see `Network ID: 777` at the top of the game window to confirm you are connected to Obscuro Testnet.
1. MetaMask will open and ask to connect your account. Click `Next` then click `Connect`.
1. Approve the payment of 1 OBX to play the game (this will be added to the prize pool) by clicking the `Approve game fee` button.
...
1. Open MetaMask to check the progress of your guess transaction in the `Activity` tab of MetaMask.
...
1. When making a guess nothing appears to happen. I win neither the prize pool nor does the prize pool increase. MetaMask shows a failed transaction in the `Activity` tab.
...
1. There is no confirmation you have made a correct guess.
    ...
    * **Workaround**: look at the value of the prize pot and your balance. When you correctly guess the secret number the prize pot will reduce to zero and your account balance will increase accordingly.
