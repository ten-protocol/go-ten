...
func CreateDBFromConfig(nodeID uint64, cfg config.EnclaveConfig) (ethdb.Database, error) {
	...
	if cfg.UseInMemoryDB {
		common.LogWithID(nodeID, "UseInMemoryDB flag is true, data will not be persisted. Creating in-memory database...")
		return getInMemDB()
	}
	...
	common.LogWithID(nodeID, fmt.Sprintf("Preparing Edgeless DB connection to %s...", cfg.EdgelessDBHost))
