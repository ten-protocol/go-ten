...
func (rc *RollupChain) checkRollup(rollup *core.Rollup) ([]*types.Receipt, error) {
	...
	if err = rc.verifySig(rollup); err != nil {
		return nil, fmt.Errorf("invalid signature. Cause: %w", err)
	}
...
func (rc *RollupChain) verifySig(r *core.Rollup) error {
	...
	if r.Header.R == nil || r.Header.S == nil {
		return fmt.Errorf("missing signature on rollup")
	}

	pubKey, err := rc.storage.FetchAttestedKey(r.Header.Agg)
	if err != nil {
		return fmt.Errorf("could not retrieve attested key for aggregator %s. Cause: %w", r.Header.Agg, err)
	}

	verifies := ecdsa.Verify(pubKey, h[:], r.Header.R, r.Header.S)
	if !verifies {
		return fmt.Errorf("signature did not verify using ECDSA")
	}
	return nil
}
