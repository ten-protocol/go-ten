...
func (rc *RollupChain) isValidBatch(batch *core.Batch, rootHash common.L2RootHash, txReceipts []*types.Receipt, depositReceipts []*types.Receipt, stateDB *state.StateDB) bool {
	...
	if !rc.isValidSequencerSig(batch.Hash(), &batch.Header.Agg, batch.Header.R, batch.Header.S) {
		return false
	}
...
func (rc *RollupChain) processRollups(rollups []*core.Rollup, block *common.L1Block) error {
