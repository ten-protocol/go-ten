...
func NewEnclave(
	...
	blockchain := ethchainadapter.NewEthChainAdapter(big.NewInt(config.TenChainID), registry, storage, gethEncodingService, *config, logger)
	...
	subscriptionManager := events.NewSubscriptionManager(storage, registry, config.TenChainID, logger)
