...
func (s *sequencer) produceBatch(
	...
	if err := s.StoreExecutedBatch(ctx, cb.Batch, cb.TxExecResults, cb.SyntheticTxs); err != nil {
...
func (s *sequencer) StoreExecutedBatch(ctx context.Context, batch *core.Batch, txResults []*core.TxExecResult, syntheticTxs []*core.SyntheticTx) error {
	...
	if err := s.storage.StoreExecutedBatch(ctx, batch.Header, txResults, syntheticTxs); err != nil {
		return fmt.Errorf("failed to store batch. Cause: %w", err)
	}
