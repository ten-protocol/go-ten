...
func (h *host) TenConfig() (*common.TenNetworkInfo, error) {
	...
	return &common.TenNetworkInfo{
		...
		L2Bridge:                  h.publicSystemContracts["EthereumBridge"],
		L1CrossChainMessenger:     importantContractAddresses.L1CrossChainMessenger,
		L2CrossChainMessenger:     h.publicSystemContracts["CrossChainMessenger"],
...
func (h *host) SequencerAttestations() ([]*common.PublicAttestationReport, error) {
	attestations, err := h.services.Enclaves().FetchAttestations(context.Background())
	if err != nil {
