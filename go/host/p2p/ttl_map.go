...
// ttlMap is a map in which the values that are not updated within TTL are removed/reset
// this is effective for :
// - count errors, clears if no errors happen within TTL - > no errors for the TTL period
// - count messages, clears if no messages are received within TTL -> error for the TTL period
type ttlMap struct {
