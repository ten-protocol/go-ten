...
func StartServer(address string, nodeID common.Address, genesisJSON []byte, collector StatsCollector) error {
	lis, err := net.Listen("tcp", address)
	if err != nil {
		return fmt.Errorf("enclave RPC server could not listen on port: %w", err)
	}
	enclaveServer := server{enclave: NewEnclave(nodeID, true, genesisJSON, collector), rpcServer: grpc.NewServer()}
...
func (s *server) ProduceGenesis(_ context.Context, request *generated.ProduceGenesisRequest) (*generated.ProduceGenesisResponse, error) {
	genesisRollup := s.enclave.ProduceGenesis(common.BytesToHash(request.BlockHash))
