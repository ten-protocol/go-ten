...
func (c *EnclaveRPCClient) ProduceGenesis(blkHash gethcommon.Hash) nodecommon.BlockSubmissionResponse {
	timeoutCtx, cancel := context.WithTimeout(context.Background(), c.config.EnclaveRPCTimeout)
	defer cancel()
...
func (c *EnclaveRPCClient) RoundWinner(parent common.L2RootHash) (nodecommon.ExtRollup, bool, error) {
	timeoutCtx, cancel := context.WithTimeout(context.Background(), c.config.EnclaveRPCTimeout)
	defer cancel()
...
func (c *EnclaveRPCClient) GetTransaction(txHash gethcommon.Hash) *nodecommon.L2Tx {
	timeoutCtx, cancel := context.WithTimeout(context.Background(), c.config.EnclaveRPCTimeout)
	defer cancel()
...
func (c *EnclaveRPCClient) GetRollup(rollupHash common.L2RootHash) *nodecommon.ExtRollup {
	timeoutCtx, cancel := context.WithTimeout(context.Background(), c.config.EnclaveRPCTimeout)
	defer cancel()
