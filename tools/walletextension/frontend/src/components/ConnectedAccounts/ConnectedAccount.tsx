...
export default function ConnectedAccount({ address, active }: Props) {
    const { isAuthenticated, isAuthenticatedLoading, authenticateAccount } =
        useTenChainAuth(address);
        ...
        <div className="bg-white dark:bg-neutral-800 p-3 rounded-md mb-3 flex gap-4 justify-between items-center">
            ...
            {isAuthenticatedLoading ? (
                ...
                <div className="flex items-center gap-4">
                    <Badge className={authClasses}>{isAuthenticated ? 'TRUE' : 'FALSE'}</Badge>
                    {!isAuthenticated && (
                        <Tooltip>
                            ...
                            <TooltipContent>Authenticate Account</TooltipContent>
                        </Tooltip>
                    )}
                </div>
            )}
