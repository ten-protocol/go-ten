...
go-obscuro/integration/gethnetwork/main/geth --numNodes=1 --startPort=12000 --websocketStartPort=12100 --prefundedAddrs=$PUB_KEY_ONE_ADDR,$PUB_KEY_TWO_ADDR,$PUB_KEY_THREE_ADDR > ./run_logs.txt 2>&1 &
...
sudo docker run -e OE_SIMULATION=1 --privileged -v /dev/sgx:/dev/sgx -p 127.0.0.1:11000:11000/tcp obscuro_enclave --hostID 1 --address :11000 --willAttest=false > ./run_logs.txt 2>&1 &
#sudo docker run -e OE_SIMULATION=1 --privileged -v /dev/sgx:/dev/sgx -p 127.0.0.1:11001:11000/tcp obscuro_enclave --hostID 2 --address :11000 --willAttest=false > ./run_logs.txt 2>&1 &
